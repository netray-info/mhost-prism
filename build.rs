fn main() {
    ...
    if !dist.exists() {
        // In debug mode, create a placeholder so development builds work
        // without requiring a frontend build. The dev workflow uses
        // Vite's dev server on :5173 with a proxy to :8080.
